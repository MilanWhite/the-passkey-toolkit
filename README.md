...
* `POST /api/passkeys/register/options` → `PublicKeyCredentialCreationOptionsJSON`
* `POST /api/passkeys/register/verify`  → `{ "verified": boolean }`
* `POST /api/passkeys/authenticate/options` → `PublicKeyCredentialRequestOptionsJSON`
* `POST /api/passkeys/authenticate/verify`  → `{ "verified": boolean }`
